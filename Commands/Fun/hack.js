...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (Member.user.id === message.author.id)
      return message.channel.send(`You know your password right -_-`);

    let embed = new MessageEmbed()
      .setColor(Color)
      .setTitle(`Hack Status: Completed`)
      .setDescription(`Name: ${Member.user.username} | ID: ${Member.user.id}`)
      .setFooter(`Jk Don't Take It Serious Its Just Random Generated Strings!`)
      .setTimestamp();

    await message.channel.send(
      `Hacking Started! Hacking ${Member.user.username}`
    );
